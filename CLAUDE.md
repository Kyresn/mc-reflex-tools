...
- `fabric` — loader entrypoint (`McReflexToolsFabricClient`), the Vulkan context resolver + validator, config (`config/ModConfig`), the settings UI (`gui/ReflexOptionsScreen`), and all Mixins.
...
The `/reflex` client command (`status`, `mode off|on|boost`, `framelimit <fps>`, `debug 0|1`) and the **Options → Video Settings → NVIDIA Reflex Tool** screen mutate that config at runtime and persist it; the per-tick apply picks the change up on the next frame. The screen is an `OptionsSubScreen` (see `docs/architecture.md` AD-019) — its widgets write `ModConfig` through the same path the command uses, and nothing in it calls Streamline directly.

The frame-limit input box applies on Enter, focus loss, or screen close. Do **not** apply it from `EditBox.setResponder`: that fires per keystroke and would push partial numbers like `7` and `70` to the driver as `frameLimitUs`.
...
- Releasing is a tag push: `git tag v$mod_version && git push origin v$mod_version`. `release.yml` refuses a tag that does not match `mod_version` and publishes a **release** (not a prerelease — GitHub counts only normal releases on the repository sidebar, so a prerelease degrades to a bare tag entry) with the jar attached. Pushes to `main` are built and uploaded as a run artifact by `build.yml`.
