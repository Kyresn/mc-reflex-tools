...
The `/reflex` client command (`status`, `mode off|on|boost`, `framelimit <fps>`, `debug 0|1`) mutates that config at runtime and persists it; the per-tick apply picks the change up on the next frame.
